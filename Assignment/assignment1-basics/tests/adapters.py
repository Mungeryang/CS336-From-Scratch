    ...
    mha.load_state_dict(
        {
            ...
            "k_proj.weight": k_proj_weight,
            "v_proj.weight": v_proj_weight,
            "output_proj.weight": o_proj_weight, # 这是是一个槽点: output_proj是固定的，我一开始写成 o_proj 后 attention 可以测试过但是下面 block 测试失败
        }
    )
...
def run_transformer_lm(
    ...
    # 槽点: Transformer_LM里的组件命名一定要规范地参考上文注释，不能自己随意起名字，否则测试不通过
...
def run_cross_entropy(
    ...
    """
    trans = inputs - inputs.max(dim=-1, keepdim=True).values
    log = torch.log(torch.exp(trans).sum(dim=-1))
    targets_logits = trans.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)
    loss = -targets_logits + log
    return loss.mean()
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from cs336_basics.train_functioin import gradient_clipping
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.train_functioin import get_lr_cosine_schedule
...
def get_tokenizer(
    ...
    from cs336_basics.tokenizer_fix import BPETokenizer
    
    
    #return BPETokenizer(vocab,merges,special_tokens)
    
    return Tiny_BPETokenizer(vocab,merges,special_tokens)
